...
<body>
    ...
    <div class="bg-white-100 flex flex-col items-center min-h-screen pt-8">
        <h1 class="text-3xl text-center text-green-800 font-bold mb-6">{{$quizName}}
        </h1>
        <h2 class="text-2xl text-center text-green-700 font-bold mb-6">Total Questions No.{{session('currentQuiz')['totalMcq']}}
        </h2>

        <h2 class="text-xl text-center text-green-700 font-bold mb-6">{{session('currentQuiz')['currentMcq']}} of {{session('currentQuiz')['totalMcq']}}
        </h2>

        <div class="my-4 p-5 bg-white shadow-xl rounded-xl w-140">
            <h3 class="text-green-700 font-bold text-2xl mb-1 ">{{$mcqData->question}}</h3>
